defmodule CPSolver.Propagator.Sum do
  ...
  def filter(all_vars, %{sum_fixed: sum_fixed, unfixed_vars: unfixed_vars} = _state) do
    unfixed_vars = Enum.filter(all_vars, fn v -> MapSet.member?(unfixed_vars, Interface.id(v)) end)
    #unfixed_vars = Map.values(unfixed_vars)
    {sum_min, sum_max} = sum_min_max(sum_fixed, unfixed_vars)
    filter_impl(unfixed_vars, sum_fixed, sum_min, sum_max)
  end

  defp filter_impl(_variables, _sum_fixed, sum_min, sum_max) when sum_min > 0 or sum_max < 0 do
    :fail
  end

  defp filter_impl(variables, sum_fixed, sum_min, sum_max) do
    {new_sum_min, new_sum_max} =
      Enum.reduce(variables, {sum_fixed, sum_fixed}, fn v, {s_min, s_max} ->
        min_v = min(v)
        max_v = max(v)
    ...
    ((new_sum_min != sum_min ||
        new_sum_max != sum_max) && filter_impl(variables, sum_fixed, new_sum_min, new_sum_max)) ||
      :ok
  end
