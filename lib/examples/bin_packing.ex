defmodule CPSolver.Examples.BinPacking do
  ...
  def check_solution(result, item_weights, max_capacity) do
    ...
    %{loads: bin_loads, bin_contents: bin_contents} = solution_to_bin_content(best_solution, item_weights, max_capacity)
    ## Loads do no exceed max capacity
    true = Enum.all?(bin_loads, fn l -> l <= max_capacity end)
    ## All items are placed into bins
    all_item_indices = Enum.reduce(tl(bin_contents), hd(bin_contents), fn bin_items, acc ->
    ...
    true = Enum.all?(Enum.zip(bin_loads, bin_contents), fn {load, items} ->
  ...
  def solution_to_bin_content(solution, item_weights, _max_capacity) do
    ...
    bin_contents =
      ...
      |> Enum.flat_map(fn bin ->
        bin_content = Enum.reduce(Enum.with_index(bin, 0), MapSet.new(), fn {i, pos}, acc -> i == 0 && acc || MapSet.put(acc, pos) end)
