defmodule SudokuRosetta do
  ...
  defp is_not_allowed_columns(grid),
    do: values_all_columns(grid) |> Enum.any?(&is_not_allowed_values/1)

  defp is_not_allowed_groups(grid),
    do: values_all_groups(grid) |> Enum.any?(&is_not_allowed_values/1)
  ...
  defp positions_not_in_grid(grid) do
    keys = Map.keys(grid)
    for x <- 1..9, y <- 1..9, {x, y} not in keys, do: {x, y}
  end
  ...
  defp potentials(grid),
    do: List.flatten(for x <- positions_not_in_grid(grid), do: potentials(x, grid))
  ...
  defp values_all_columns(grid) do
    for x <- 1..9, do: for(y <- 1..9, do: {x, y}) |> potentials_values(grid)
  end

  defp values_all_groups(grid) do
    [[g1, g2, g3], [g4, g5, g6], [g7, g8, g9]] =
      for x <- [1, 4, 7], do: values_all_groups(x, grid)
  ...
  defp values_all_rows(grid) do
    for y <- 1..9, do: for(x <- 1..9, do: {x, y}) |> potentials_values(grid)
  end
  ...
  defp solve_all_sure_values(grid),
    do: for({position, [value]} <- potentials(grid), do: {position, value})
