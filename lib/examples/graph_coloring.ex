defmodule CPSolver.Examples.GraphColoring do
  ...
  def solve(data) do
    ...
    edge_color_constraints = Enum.map(data.edges, fn [v1, v2] -> NotEqual.post(Enum.at(color_vars, v1), Enum.at(color_vars, v2)) end)
    model = %{
  ...
  defp parse_instance(instance) do
    ...
    edges = Enum.map(edge_lines, fn e -> String.split(e, " ") |> Enum.map(&String.to_integer/1) end)
