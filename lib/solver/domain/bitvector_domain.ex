defmodule CPSolver.BitVectorDomain do
  ...
  def fail?({{:bit_vector, _zero_based_max, _atomics_ref} = _bit_vector, _offset} = domain) do
    size(domain) == 0
  end
  ...
  defp set_fail({{:bit_vector, _zero_based_max, atomics_ref} = _bit_vector, _offset} = _domain) do
